# tools/step75_final_append_only_fix.py
# ------------------------------------------------------------
# Step75: 최종 append-only DOM 보정
#
# 원칙:
# - 기존 Python 함수/제목/메모/시간표 렌더링 코드를 건드리지 않음
# - 깨진 app.py이면 컴파일 가능한 백업으로 먼저 복구
# - 최종 보정 코드는 파일 맨 끝에만 추가
#
# 수정:
# 1) 달력 글자 세로배치 방지
# 2) 제목 오른쪽 현재시각 배지 표시
# 3) 메모의 literal <span style="color:#...">...</span> 실제 색상 표시
# 4) 시간표 디자인 유지
# ------------------------------------------------------------
...
def choose_base_text(raw: str):
    ...
    for b in candidates:
        # 실패한 step71~74 백업은 대부분 같은 오류를 갖고 있을 수 있어도 컴파일 검사로 걸러냄
...
def main():
    print("====================================================")
    print("Step75 final append-only DOM fix")
    print("====================================================")
    print(f"[ROOT] {ROOT}")
    print()
    ...
    try:
        ...
        # 파일 끝에만 삽입: 중간 except/함수 블록이 깨질 가능성 제거
        text = text.rstrip() + "\n\n" + PATCH_BLOCK.strip("\n") + "\n"
    except Exception as e:
        print("[오류] 패치 실패")
        print(e)
    ...
    print("[완료] Step75 패치 저장")

    print()
    print("[변경 요약]")
    print(f"- 복구 백업 사용: {restored_from}")
    print(f"- Step70~75 이전 블록 제거: {removed_blocks}")
