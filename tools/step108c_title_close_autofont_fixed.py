...
BACKUP_DIR = ROOT / "tools" / "_step108c_backups"

CSS_BEGIN = "# >>> STEP108C_TITLE_AND_AUTOFONT_CSS_BEGIN"
CSS_END = "# >>> STEP108C_TITLE_AND_AUTOFONT_CSS_END"
AUTO_BEGIN = "# >>> STEP108C_AUTOFONT_BEGIN"
AUTO_END = "# >>> STEP108C_AUTOFONT_END"
...
"        .header-container, .step99-header-container {{ margin-bottom: -72px !important; }}",
"        div:has(> .header-container), div:has(> .step99-header-container) {{ margin-bottom: -72px !important; }}",
...
ADAPTIVE_FONT_BLOCK = "\n".join([
AUTO_BEGIN,
"            # Step108c: 칸 내용 길이에 따라 자동 줄바꿈 + 폰트 1px 단위 축소",
...
def main() -> int:
    print("=" * 60)
    print("Step108c 제목 위치/크기 + 시간표 칸 자동 폰트 축소")
    ...
    backup = BACKUP_DIR / f"app_before_step108c_{ts}.py"
    ...
    # 안전장치: 이전 실패 때 생겼을 수 있는 붙은 줄 교정
    text = text.replace('line_height = "1.17"if period == "학사일정":', 'line_height = "1.17"\n            if period == "학사일정":')

    if "step108_plain = re.sub" not in text:
        # 원래 font 설정 블록은 대개 이후에 'if period == "학사일정":'이 이어짐.
        # 그 if 앞까지 포함해 교체하여 뒤에 코드가 붙는 문제를 막음.
        pattern_with_next_if = re.compile(
            r"            font_sz_str\s*=\s*\"14px\"\s*\n"
            r"            line_height\s*=\s*\"1\.2\"\s*\n"
            r"(?=            if period == \"학사일정\":)",
            re.S,
        )
        text2, n = pattern_with_next_if.subn(ADAPTIVE_FONT_BLOCK, text, count=1)
        ...
        if n == 0:
            ...
            text2, n = pattern_simple.subn(ADAPTIVE_FONT_BLOCK, text, count=1)
    ...
    for marker in insert_markers:
        if marker in text:
            ...
            inserted = True
            print(f"[수정] {marker} 뒤에 Step108c CSS를 추가했습니다.")
            break
    ...
    tmp = APP.with_suffix(".step108c_test.py")
    write_text(tmp, text)
