# tools/step85_school_fix_period_growth_query_calendar.py
# ------------------------------------------------------------
# Step85: 학교 경로용 현재교시 늘어남/조회칸/달력 중앙정렬 수정
#
# 수정:
# 1) 현재교시 표시가 반복 실행될 때 <br>이 누적되어 행이 계속 늘어나는 문제 제거
# 2) 조회 행 내용이 길면 칸 안에서 글자 크기를 줄이고 줄바꿈 처리
# 3) 달력 버튼을 실제 버튼 중앙 overlay 방식으로 재보정
# 4) Step84의 문제 DOM 블록 제거 후 안정 DOM 블록으로 대체
# 5) Supabase SSL fallback 유지/삽입
# 6) 러블리 핑크 테마 추가 재시도
#
# 학교 경로:
...
def backup_current():
    b = APP.with_name(f"{APP.stem}_before_step85_period_growth_fix_{STAMP}{APP.suffix}")
...
def ensure_ssl_fallback(text: str):
    if "# [STEP84_SUPABASE_SSL_FALLBACK_START]" in text or "# [STEP85_SUPABASE_SSL_FALLBACK_START]" in text:
...
def main():
    print("====================================================")
    print("Step85 period growth/query/calendar fix")
    print("====================================================")
    print(f"[ROOT] {ROOT}")
    print()
    ...
    try:
        ...
        text, ssl_status = ensure_ssl_fallback(text)
    ...
    print("[완료] Step85 패치 저장")
    print()
    print("[변경 요약]")
    print(f"- 기존 Step82/83/84/85 DOM 블록 제거: {removed}")
    print(f"- Supabase SSL fallback: {ssl_status}")
    print(f"- 러블리 핑크 테마 dict 처리: {theme_dict_status}")
    print(f"- 러블리 핑크 옵션 처리: {option_changes}")
    print("- 현재교시 표시 행 늘어남 방지")
    print("- 조회 행 긴 내용 칸 맞춤 글자 축소")
    print("- 달력 버튼 중앙 overlay 재보정")
    print()
    print("실행:")
    print("python -m streamlit run mobile\\app.py")
    print()
    print("확인할 것:")
    print("1. 현재교시 행이 더 이상 세로로 계속 늘어나지 않는지")
    print("2. 조회 행 긴 내용이 칸 안에서 작게 표시되는지")
    print("3. 달력 글자가 중앙에 오는지")
    print("4. 현재 시각 기준 요일/교시/칸 표시가 정상인지")
    input("엔터를 누르면 종료합니다.")
