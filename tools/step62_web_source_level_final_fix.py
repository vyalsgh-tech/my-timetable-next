# tools/step62_web_source_level_final_fix.py
# ------------------------------------------------------------
# Step62: 웹뷰어 소스 레벨 최종 수정
# ------------------------------------------------------------
...
def backup(path: Path):
    if not path.exists():
        return None
    b = path.with_name(f"{path.stem}_before_step62_web_source_fix_{STAMP}{path.suffix}")
...
def remove_old_blocks(text: str):
    ...
    for start, end in OLD_BLOCK_MARKERS:
        while start in text and end in text:
            text, did = remove_block(text, start, end)
            if did:
                removed += 1
            else:
                break
...
def ensure_helpers(text: str):
    text, _ = remove_block(text, HELPER_START, HELPER_END)
    ...
    if pos == -1:
        ...
        if pos2 == -1:
            raise RuntimeError("normalize_text 또는 URL 파라미터 섹션 위치를 찾지 못했습니다.")
        return text[:pos2] + HELPERS + "\n\n" + text[pos2:], "before_section6"
    next_section = text.find("\n# =========================================================", pos + len(marker))
    if next_section == -1:
        raise RuntimeError("normalize_text 함수 끝 위치를 찾지 못했습니다.")
    return text[:next_section] + "\n\n" + HELPERS + text[next_section:], "after_normalize_text"


def ensure_css(text: str):
    while CSS_START in text and CSS_END in text:
        text, _ = remove_block(text, CSS_START, CSS_END)
    idx = text.find("</style>")
    if idx == -1:
...
def ensure_clock_call(text: str):
    text, _ = remove_block(text, CLOCK_CALL_START, CLOCK_CALL_END)
...
def patch_timetable_frame(text: str):
    changed = 0
    if "class='timetable-frame'" in text:
        return text, 0

    old1 = 'html_parts.append(f"<div style=\'width:100%; overflow-x:auto; background-color:{t[\'grid\']}; border-radius:4px;\'><table class=\'mobile-table\'>")'
    old2 = 'html_parts.append("<table class=\'mobile-table\'>")'
    new_start = 'html_parts.append("<div class=\'timetable-frame\'><div class=\'timetable-scroll\'><table class=\'mobile-table\'>")'

    if old1 in text:
        text = text.replace(old1, new_start, 1)
        changed += 1
    elif old2 in text:
        text = text.replace(old2, new_start, 1)
        changed += 1
    else:
        # fallback: regex
        pat = r'html_parts\.append\(f"<div style=\'width:100%; overflow-x:auto; background-color:\{t\[\'grid\'\]\}; border-radius:4px;\'><table class=\'mobile-table\'>"\)'
        text, n = re.subn(pat, new_start, text, count=1)
        changed += n

    if changed == 0:
        raise RuntimeError("시간표 시작 html_parts.append 위치를 찾지 못했습니다.")

    old_end = 'html_parts.append("</table></div>")'
    new_end = 'html_parts.append("</table></div><div class=\'timetable-bottom-shadow\'></div></div>")'
    if old_end in text:
...
def main():
    print("==============================================")
    print("Step62 web source-level final fix")
    print("==============================================")
    print(f"[ROOT] {ROOT}")
    print()
    if not APP.exists():
    ...
    original = text
    try:
        text, removed_old = remove_old_blocks(text)
    ...
    if text != original:
        ...
        print("[완료] Step62 패치 저장")
    else:
        print("[안내] 변경 없음")
    print()
    print("[변경 요약]")
    print(f"- 이전 DOM 보정 블록 제거: {removed_old}")
    ...
    print("3. 시간표 바깥쪽의 촌스러운 사각 테두리 느낌이 줄었는지")
    print("4. 메모장에 중요 메모 / 일반 메모 / 완료 메모 소제목이 생겼는지")
    print("5. 각 소제목 클릭 시 접힘/펼침이 되는지")
    print("6. 메모 넘버링이 계속 없는지")
    input("엔터를 누르면 종료합니다.")
