...
BACKUP_DIR = ROOT / "tools" / "_step102_backups"

HELPER_BEGIN = "# >>> STEP102_FINAL_HELPERS_BEGIN"
HELPER_END = "# >>> STEP102_FINAL_HELPERS_END"
CSS_BEGIN = "# >>> STEP102_FINAL_CSS_AND_CLOCK_BEGIN"
CSS_END = "# >>> STEP102_FINAL_CSS_AND_CLOCK_END"
...
HELPER_BLOCK = "\n".join([
HELPER_BEGIN,
"def step102_final_theme_fix(theme, theme_idx=0):",
"    t2 = dict(theme or {})",
...
"    elif any(k in name for k in ['모노', 'mono', 'gray', 'grey', '그레이', '회색']):",
"        put(bg='#2f3338', top='#252a30', grid='#8b949e', head_bg='#3f4650', head_fg='#f8fafc', per_bg='#4b5563', per_fg='#f8fafc', cell_bg='#f8fafc', cell_fg='#111827', lunch_bg='#eef2f7', hl_per='#64748b', hl_cell='#e2e8f0', text='#f8fafc', table_shell='#353b43', button_primary_bg='#64748b', button_primary_fg='#ffffff', button_secondary_bg='#374151', button_secondary_fg='#f8fafc', button_border='#8b949e')",
...
"    return t2",
"",
"def step102_current_clock_info():",
"    try:",
...
"",
"def step102_render_memo_html(value):",
...
"        background: {t.get('button_secondary_bg', t.get('top', '#ffffff'))} !important;",
"        color: {t.get('button_secondary_fg', t.get('text', '#0f172a'))} !important;",
"        border: 1px solid {t.get('button_border', t.get('grid', '#94a3b8'))} !important;",
"        box-shadow: 0 2px 8px rgba(0,0,0,.14) !important;",
"        line-height: 1.15 !important;",
"    }}",
"    .step102-clock-time {{ font-size: 13px !important; font-weight: 800 !important; }}",
"    .step102-clock-label {{ font-size: 11px !important; margin-top: 2px !important; opacity: .9 !important; }}",
...
"        color: {t.get('button_secondary_fg', t.get('text', '#0f172a'))} !important;",
"        background-color: {t.get('button_secondary_bg', t.get('top', '#ffffff'))} !important;",
"        border-color: {t.get('button_border', t.get('grid', '#94a3b8'))} !important;",
"    }}",
...
"        background-color: {t.get('button_primary_bg', t.get('hl_per', '#2563eb'))} !important;",
"    }}",
...
")",
"_step102_time, _step102_label = step102_current_clock_info()",
"st.markdown(",
"    f\"<div class='step102-clock-badge'><div class='step102-clock-time'>{_step102_time}</div><div class='step102-clock-label'>{_step102_label}</div></div>\",",
"    unsafe_allow_html=True,",
")",
CSS_END,
]) + "\n"
...
def main() -> int:
    print("=" * 60)
    print("Step102 다크/모노톤 대비, 메모 색상 span, 현재시각 복원")
    ...
    backup = BACKUP_DIR / f"app_before_step102_{ts}.py"
    ...
    text = re.sub(r"\n\s*t\s*=\s*step102_final_theme_fix\(t,\s*st\.session_state\.theme_idx\)\s*", "\n", text)
    ...
    if "t = step99_apply_viewer_theme_palette(t)" in text:
        ...
        text = text.replace(marker, marker + "\nt = step102_final_theme_fix(t, st.session_state.theme_idx)", 1)

    # 메모 색상 span이 그대로 노출되는 문제 수정
    old = 'f"<div>{prefix}{html.escape(text).replace(chr(10), \'<br>\')}</div>"'
    new = 'f"<div>{prefix}{step102_render_memo_html(text)}</div>"'
    if old in text:
        text = text.replace(old, new, 1)
        print("[수정] 메모 색상 span 렌더링을 실제 색상 표시 방식으로 변경했습니다.")
    else:
        # 공백 차이가 있는 경우를 위한 보조 정규식
        pattern = r'f"<div>\{prefix\}\{html\.escape\(text\)\.replace\(chr\(10\),\s*[\'"]<br>[\'"]\)\}</div>"'
        text2, n = re.subn(pattern, new, text, count=1)
        if n:
            text = text2
            print("[수정] 메모 색상 span 렌더링을 실제 색상 표시 방식으로 변경했습니다.")
        else:
            print("[주의] 메모 html.escape 렌더링 줄을 찾지 못했습니다.")

    # CSS/현재시각은 dashboard 함수 정의 직전에 삽입
    dash_def = "@st.fragment\ndef display_dashboard():"
    if dash_def in text:
        text = text.replace(dash_def, CSS_CLOCK_BLOCK + "\n" + dash_def, 1)
        print("[수정] dashboard 정의 직전에 현재시각 및 대비 CSS를 삽입했습니다.")
    elif "\ndisplay_dashboard()" in text:
        text = text.replace("\ndisplay_dashboard()", "\n" + CSS_CLOCK_BLOCK + "\ndisplay_dashboard()", 1)
        print("[주의] dashboard 정의부를 찾지 못해 호출 직전에 삽입했습니다.")
    else:
        text += "\n" + CSS_CLOCK_BLOCK
        print("[주의] 삽입 위치를 찾지 못해 파일 끝에 추가했습니다.")

    tmp = APP.with_suffix(".step102_test.py")
    write_text(tmp, text)
