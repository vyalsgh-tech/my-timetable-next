# tools/step86_school_scroll_row_height_fix.py
# ------------------------------------------------------------
# Step86: 학교 경로용 스크롤 튐/행 높이/조회칸/달력 보정
#
# 수정:
# 1) 창 스크롤이 최상단으로 반복 당겨지는 현상 완화
#    - DOM 보정 전후 scrollTop을 보존
#    - 잦은 setInterval을 1분 간격으로 축소
#
# 2) 칸 높이 문제 보정
#    - 기본 시간표 셀 높이를 균일하게 설정
#    - 긴 글자는 글자 크기를 먼저 줄임
#    - 그래도 길면 해당 행만 자연스럽게 늘어나도록 유지
#
# 3) 조회 행 긴 내용
#    - 글자 크기/자간/줄간격을 줄여 칸 안에서 표시
#
# 4) 현재교시 표시
#    - <br> 추가 금지
#    - 배지는 absolute 방식이라 행 높이를 늘리지 않음
#
# 5) 달력 버튼 중앙 overlay 재보정
#
# 학교 경로:
...
def backup_current():
    b = APP.with_name(f"{APP.stem}_before_step86_scroll_row_fix_{STAMP}{APP.suffix}")
...
def ensure_ssl_fallback(text: str):
    if (
        "# [STEP84_SUPABASE_SSL_FALLBACK_START]" in text
        or "# [STEP85_SUPABASE_SSL_FALLBACK_START]" in text
        or "# [STEP86_SUPABASE_SSL_FALLBACK_START]" in text
    ):
...
def main():
    print("====================================================")
    print("Step86 scroll/row height/query/calendar fix")
    print("====================================================")
    print(f"[ROOT] {ROOT}")
    print()
    ...
    print("[완료] Step86 패치 저장")
    print()
    print("[변경 요약]")
    print(f"- 기존 Step82/83/84/85/86 DOM 블록 제거: {removed}")
    print(f"- Supabase SSL fallback: {ssl_status}")
    print(f"- 러블리 핑크 테마 dict 처리: {theme_dict_status}")
    print(f"- 러블리 핑크 옵션 처리: {option_changes}")
    print("- 스크롤 최상단 당김 방지")
    print("- 행 높이 기본 균일화 + 긴 글자 자동 축소")
    print("- 조회 행 글자 크기 축소")
    print("- 현재교시 배지를 absolute 방식으로 변경")
    ...
    print()
    print("확인할 것:")
    print("1. 스크롤이 최상단으로 반복 당겨지지 않는지")
    print("2. 각 행 높이가 기본적으로 균일한지")
    print("3. 조회 행 긴 내용이 칸 안에서 작게 표시되는지")
    print("4. 현재교시 표시가 행 높이를 늘리지 않는지")
