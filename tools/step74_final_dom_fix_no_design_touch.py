# tools/step74_final_dom_fix_no_design_touch.py
# ------------------------------------------------------------
# Step74: 최종 최소 DOM 보정
#
# 목적:
# - Step71~73 반복 실패 원인 제거
# - Python 함수/헤더 교체를 하지 않음
# - 시간표 디자인과 기존 메모 구조는 건드리지 않음
#
# 수정:
# 1) 달력 글자 세로배치 방지
# 2) 제목 오른쪽 현재시각 배지 표시
# 3) 메모의 literal <span style="color:#...">...</span> 을 실제 색상으로 표시
# ------------------------------------------------------------
...
def insert_patch_block(text: str):
    lines = text.splitlines()
    idx, pos = find_html_render_insert_index(lines)
    lines[idx:idx] = PATCH_BLOCK.strip("\n").splitlines()
    return "\n".join(lines) + "\n", pos


def main():
    print("====================================================")
    print("Step74 final DOM fix - no design touch")
    print("====================================================")
    print(f"[ROOT] {ROOT}")
    print()
    ...
    try:
        text, restored_from = choose_base_text(raw)
        text, removed_blocks, removed_calls = remove_old_blocks(text)
        text, literal_changes = patch_calendar_literal(text)
        text, insert_pos = insert_patch_block(text)
    except Exception as e:
        print("[오류] 패치 실패")
        print(e)
    ...
    print("[완료] Step74 패치 저장")

    print()
    print("[변경 요약]")
    print(f"- 복구 백업 사용: {restored_from}")
    print(f"- Step70~74 이전 블록 제거: {removed_blocks}")
    print(f"- 단독 호출 제거: {removed_calls}")
    print(f"- 달력 문자열 보정: {literal_changes}")
    print(f"- DOM 패치 삽입 위치: {insert_pos}")
    print()
    print("확인:")
