# tools/fix_program_info_display_v3.py
# ------------------------------------------------------------
# 프로그램 정보 창에 \n이 문자 그대로 보이는 문제를 최종 수정합니다.
# 핵심: info_text = chr(10).join(lines) 사용
# 실행: python tools\fix_program_info_display_v3.py
# ------------------------------------------------------------
...
BACKUP = ROOT / "desktop" / f"timetable_before_program_info_display_fix_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pyw"
...
def ensure_import(text, module_name):
    ...
    for i, line in enumerate(src):
        ...
        if stripped.startswith('import ') or stripped.startswith('from '):
            pos = i + 1
    src.insert(pos, f'import {module_name}')
    return '\n'.join(src) + '\n'


def make_program_info_block():
    block = [
...
def patch_text(text):
    ...
    return text[:start] + make_program_info_block() + '\n' + text[end:]


def main():
    print('==============================================')
    print('프로그램 정보 표시 줄바꿈 최종 복구 v3')
    print('==============================================')
    print()
    ...
    print('[완료] 프로그램 정보 창 줄바꿈 처리를 chr(10).join 방식으로 수정했습니다.')
    print()
    print('확인:')
