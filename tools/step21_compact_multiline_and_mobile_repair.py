# tools/step21_compact_multiline_and_mobile_repair.py
...
PC_METHODS = '\n'.join([
    ...
    '        text_box = tk.Text(frame, height=2, width=44, wrap="word", bg=input_bg, fg=fg, insertbackground=fg, relief="solid", bd=1, highlightthickness=1, highlightbackground=border, font=("맑은 고딕", 10), undo=True)',
    '        text_box.pack(fill="x", expand=False)',
    '        text_box.insert("1.0", initialvalue or "")',
    '        text_box.focus_set()',
    '        guide = tk.Label(frame, text="Shift+Enter: 줄바꿈  /  Enter: 저장  /  Esc: 취소", bg=bg, fg=t.get("muted_fg", "#667085"), font=("맑은 고딕", 8), anchor="w")',
    '        guide.pack(fill="x", pady=(5, 7))',
    '        btn_frame = tk.Frame(frame, bg=bg)',
    '        btn_frame.pack(fill="x")',
    '        def current_line_count():',
    '            try:',
    '                return max(1, int(text_box.index("end-1c").split(".")[0]))',
    '            except Exception:',
    '                return 1',
    '        def resize_editor(event=None):',
    '            line_count = current_line_count()',
    '            h = max(2, min(8, line_count))',
    '            try:',
    '                text_box.configure(height=h)',
    '                win.update_idletasks()',
    '                win.geometry(f"480x{145 + h * 22}")',
    '            except Exception:',
    '                pass',
    '            return None',
    ...
    '        custom_tag = "MDGOMultilineInput" + str(id(text_box))',
    '        old_tags = text_box.bindtags()',
    '        text_box.bindtags((custom_tag,) + tuple(old_tags))',
    '        text_box.bind_class(custom_tag, "<Shift-Return>", insert_newline)',
    '        text_box.bind_class(custom_tag, "<Shift-KP_Enter>", insert_newline)',
    '        text_box.bind_class(custom_tag, "<Return>", on_return)',
    '        text_box.bind_class(custom_tag, "<KP_Enter>", on_return)',
    '        text_box.bind("<KeyRelease>", resize_editor, add="+")',
    '        text_box.bind("<Escape>", on_cancel)',
    '        win.bind("<Escape>", on_cancel)',
    '        try:',
    '            self.root.update_idletasks()',
    '            x = self.root.winfo_rootx() + max(30, (self.root.winfo_width() - 480) // 2)',
    '            y = self.root.winfo_rooty() + max(30, (self.root.winfo_height() - 190) // 2)',
    '            win.geometry(f"480x190+{x}+{y}")',
    '        except Exception:',
...
def backup(path: Path):
    if path.exists():
        b = path.with_name(f'{path.stem}_before_step21_{STAMP}{path.suffix}')
...
def strip_bad_mobile_css(text):
    lines = text.splitlines()
    out = []
    i = 0
    bad_tokens = ['white-space: nowrap !important', 'word-break: keep-all !important', 'min-width:', 'padding-left:', 'padding-right:', '.mdgo-strike']
    while i < len(lines):
        line = lines[i]
        if 'div[data-testid="stButton"]' in line and '{' in line:
            i += 1
            while i < len(lines):
                if lines[i].strip() in ('}', '}}') or '</style>' in lines[i]:
                    i += 1
                    break
                i += 1
            continue
        ...
        i += 1
    text = '\n'.join(out) + '\n'
    # CSS 제거 후 혼자 남은 잘못된 f-string 닫는 중괄호는 컴파일 오류를 만들 수 있어 제거
    return text

def compile_cleanup_mobile(text):
    for _ in range(20):
        try:
            ...
            if 0 <= idx < len(lines):
                ...
                if current in ('}', '}}') or 'min-width' in window or 'stButton' in window or 'white-space' in window:
                    del lines[idx]
                    text = '\n'.join(lines) + '\n'
                    continue
            raise
...
def patch_desktop():
    ...
    if text != original:
        ...
        print('[완료] PC compact multiline 입력창 보정')
        return True
    print('[안내] PC 변경사항 없음')
    return False
...
def patch_mobile():
    ...
    original = text
    text = strip_bad_mobile_css(text)
    # clean_view_text 교체/삽입
    ...
    # 기존 웹 보정 JS가 깨져 있을 수 있으므로 마커가 없을 때만 안전 버전 삽입
    ...
    text = compile_cleanup_mobile(text)
    if text != original:
        MOBILE_FILE.write_text(text, encoding='utf-8')
        print('[완료] 모바일 f-string/CSS 오류 복구')
        return True
...
def main():
    print('==============================================')
    print('Step21 compact multiline + mobile repair')
    print('==============================================')
    print(f'[ROOT] {ROOT}')
    print()
    try:
