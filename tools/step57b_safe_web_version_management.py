# tools/step57b_safe_web_version_management.py
# ------------------------------------------------------------
# Step57b: 웹뷰어 버전 관리 패치 안전 적용판
#
# 기존 Step57 실패 원인:
# - mobile/app.py 전체에서 마지막 import 위치를 찾다가 try 블록 내부 import 뒤에
#   helper 코드가 들어가 compile 단계에서 expected 'except' or 'finally' 오류 발생.
#
# Step57b 개선:
# 1) helper 코드는 파일 상단의 실제 import 블록 바로 뒤에만 삽입
# 2) 버전 배지는 st.set_page_config(...) 완료 직후에만 삽입
# 3) 기존 실패 패치 블록이 일부 남아 있으면 먼저 제거
# 4) compile 성공 시에만 mobile/app.py 저장
# 5) tools/bump_web_version.py 생성
#
# 실행:
#   python tools\step57b_safe_web_version_management.py
# ------------------------------------------------------------
...
def load_web_version_info():
    ...
    try:
        ...
        app_version = str(data.get("app_version") or data.get("version") or web_version)
        data_version = str(data.get("data_version") or data.get("db_version") or app_version)
        release_date = str(data.get("web_release_date") or data.get("release_date") or "")
        release_title = str(data.get("web_release_title") or data.get("release_title") or "웹뷰어")
...
def backup(path: Path):
    if not path.exists():
        return None
    b = path.with_name(f"{path.stem}_before_step57b_web_version_{STAMP}{path.suffix}")
    b.write_text(path.read_text(encoding="utf-8", errors="replace"), encoding="utf-8")
    print(f"[백업] {b}")
    return b
...
def update_version_json():
    print("[처리] data/version.json 웹뷰어 버전 키 추가/확인")
    VERSION_JSON.parent.mkdir(parents=True, exist_ok=True)
    if VERSION_JSON.exists():
        backup(VERSION_JSON)
    ...
    data["web_release_date"] = data.get("web_release_date") or TODAY
    data["web_updated_at"] = datetime.now().isoformat(timespec="seconds")
    data["web_release_title"] = data.get("web_release_title") or WEB_RELEASE_TITLE
...
def find_top_import_insertion_index(lines):
    """파일 상단의 실제 import 블록 바로 뒤만 찾는다. try 내부 import는 절대 대상으로 삼지 않는다."""
    last_import = -1
    started = False
...
def patch_mobile_app():
    print("[처리] mobile/app.py 웹버전 표시 안전 패치")
    ...
    # 기존 블록이 일부/전체 남아 있으면 제거
    text, removed_helper = remove_block(text, HELPER_START, HELPER_END)
    text, removed_call = remove_block(text, CALL_START, CALL_END)

    lines = text.splitlines()
    helper_idx = find_top_import_insertion_index(lines)
    lines.insert(helper_idx, WEB_VERSION_HELPER.strip("\n"))

    # 배지 호출은 st.set_page_config 완료 직후 삽입. 못 찾으면 helper 뒤에 넣는다.
    text = "\n".join(lines) + "\n"
    lines = text.splitlines()
    call_idx, position = find_set_page_config_end(lines)
    if call_idx is None:
        call_idx = helper_idx + len(WEB_VERSION_HELPER.strip("\n").splitlines()) + 1
        position = "after_helper"
    ...
    return {"removed_helper": removed_helper, "removed_call": removed_call, "position": position}
...
def main():
    print("==============================================")
    print("Step57b safe web version management")
    print("==============================================")
    print(f"[ROOT] {ROOT}")
    ...
    print()
    print("확인할 것:")
    print("1. 웹뷰어 화면 상단에 Web v2.1.0 배지가 보이는지")
